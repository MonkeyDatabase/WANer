...
CREATE TABLE `ops_user_info`(
                                `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '运维用户ID',
                                ...
                                `status` int(1) default 1 COMMENT '启用状态 0表示禁用 1表示启用',
                                `extra` varchar(255) DEFAULT NULL COMMENT '保留字段',
                                `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                `modified_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
                                PRIMARY KEY (`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='运维平台用户';

/******************************************/
/*   数据库全名 = sdwan_controller   */
/*   表名称 = ops_role_info   */
/******************************************/
DROP TABLE IF EXISTS `ops_role_info`;
CREATE TABLE `ops_role_info`(
                                `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '运维角色ID',
                                `name` varchar(255) NOT NULL COMMENT '运维角色英文名称',
                                `description` varchar(255) NOT NULL COMMENT '运维角色中文描述',
                                `status` int(1) default 1 COMMENT '启用状态 0表示禁用 1表示启用',
                                `extra` varchar(255) DEFAULT NULL COMMENT '保留字段',
                                `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                `modified_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
                                PRIMARY KEY (`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='运维平台角色';

/******************************************/
/*   数据库全名 = sdwan_controller   */
/*   表名称 = ops_user_role_relation   */
/******************************************/
DROP TABLE IF EXISTS `ops_user_role_relation`;
CREATE TABLE `ops_user_role_relation`(
                                         `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '用户角色关系ID',
                                         `user_id` bigint(20) NOT NULL COMMENT '运维平台用户ID',
                                         `role_id` bigint(20) NOT NULL COMMENT '运维平台角色ID',
                                         `extra` varchar(255) DEFAULT NULL COMMENT '保留字段',
                                         `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                         `modified_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
                                         PRIMARY KEY (`id`),
                                         FOREIGN KEY (`user_id`) REFERENCES `ops_user_info`(`id`),
                                         FOREIGN KEY (`role_id`) REFERENCES `ops_role_info`(`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='运维平台用户角色关系表';

/******************************************/
/*   数据库全名 = sdwan_controller   */
/*   表名称 = ops_api_info   */
/******************************************/
DROP TABLE IF EXISTS `ops_api_info`;
CREATE TABLE `ops_api_info`(
                               `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '运维平台API ID',
                               `name` varchar(255) NOT NULL COMMENT '运维平台API权限名称, 用于后续SpringSecurity注解校验',
                               `description` varchar(255) NOT NULL COMMENT '运维平台API权限中午名称',
                               `extra` varchar(255) DEFAULT NULL COMMENT '保留字段',
                               `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                               `modified_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
                               PRIMARY KEY (`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='运维平台API表';

/******************************************/
/*   数据库全名 = sdwan_controller   */
/*   表名称 = ops_role_api_relation   */
/******************************************/
DROP TABLE IF EXISTS `ops_role_api_relation`;
CREATE TABLE `ops_role_api_relation`(
                                        `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '用户角色关系ID',
                                        `role_id` bigint(20) NOT NULL COMMENT '运维平台角色ID',
                                        `api_id` bigint(20) NOT NULL COMMENT '运维平台API ID',
                                        `extra` varchar(255) DEFAULT NULL COMMENT '保留字段',
                                        `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                        `modified_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
                                        PRIMARY KEY (`id`),
                                        FOREIGN KEY (`role_id`) REFERENCES `ops_role_info`(`id`),
                                        FOREIGN KEY (`api_id`) REFERENCES `ops_api_info`(`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='运维平台角色API关系表';
